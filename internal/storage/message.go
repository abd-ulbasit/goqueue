...
//   - Metadata (who sent it, when, routing info)
...
//   - Kafka: Custom binary format, very similar to ours
//   - RabbitMQ: AMQP binary protocol
//   - SQS: JSON over HTTP (simplicity over performance)
//   - Redis Streams: RESP protocol (text-based but efficient)
//
// OUR MESSAGE FORMAT (on disk):
// ┌──────────────────────────────────────────────────────────────────────────┐
// │ HEADER (fixed 28 bytes)                                                  │
// ├──────────────────────────────────────────────────────────────────────────┤
// │ Magic (2B) │ Version (1B) │ Flags (1B) │ CRC32 (4B) │ Offset (8B)        │
// │ Timestamp (8B) │ Key Length (2B) │ Value Length (4B)                     │
// ├──────────────────────────────────────────────────────────────────────────┤
// │ BODY (variable)                                                          │
// ├──────────────────────────────────────────────────────────────────────────┤
...
//   - Future versions can read old formats by checking this byte
//
...
//   - IMPORTANT: CRC covers offset+timestamp+key+value, NOT magic/version/flags
//
...
const (
	...
	// Magic(2) + Version(1) + Flags(1) + CRC(4) + Offset(8) + Timestamp(8) + KeyLen(2) + ValueLen(4) = 30
	HeaderSize = 30
...
//	[24:26] Key length (uint16, big-endian)
//	[26:30] Value length (uint32, big-endian)
//	[30:30+keyLen] Key bytes
//	[30+keyLen:end] Value bytes
//
// WHY BIG-ENDIAN?
...
func (m *Message) Encode() ([]byte, error) {
	...
	binary.BigEndian.PutUint16(buf[24:26], uint16(len(m.Key)))

	// Value length - needed to know where message ends
	binary.BigEndian.PutUint32(buf[26:30], uint32(len(m.Value)))
...
func Decode(data []byte) (*Message, error) {
	...
	// -------------------------------------------------------------------------
	// We need at least HeaderSize bytes to read the header
	...
	if version != FormatVersion {
		return nil, fmt.Errorf("%w: got version %d, only support version %d",
			ErrUnsupportedVersion, version, FormatVersion)
	}

	// -------------------------------------------------------------------------
	// STEP 4: Extract header fields
	// -------------------------------------------------------------------------
	flags := data[3]
	...
	keyLen := binary.BigEndian.Uint16(data[24:26])
	valueLen := binary.BigEndian.Uint32(data[26:30])

	// -------------------------------------------------------------------------
	// STEP 5: Validate lengths
	// -------------------------------------------------------------------------
	// The claimed key+value length must match actual data size
	...
	// We recalculate CRC over the same range and compare
	...
	// -------------------------------------------------------------------------
	// STEP 7: Extract key and value
	// -------------------------------------------------------------------------
	// We make copies of key and value to avoid holding reference to original buffer
	// This is important because the original buffer might be reused
	var key []byte
	if keyLen > 0 {

...
**M21: Security Features Deployed to EKS**:
- ✅ **TLS for Client APIs**: HTTPS enabled on port 8080
  - Self-signed certificates auto-generated by Helm
  - TLS certificate and CA mounted from Kubernetes secrets
- ✅ **mTLS for Cluster Communication**: Inter-node traffic encrypted
  - Cluster TLS certificates for goqueue-0, -1, -2
  - Client certificate verification enabled
- ✅ **API Key Authentication**: Root API key configured
  - Requests without valid API key rejected with 401
  - Health endpoints (/healthz, /readyz, /metrics) bypass auth
- ✅ **RBAC Enabled**: Role-based access control active

**M23: Schema Registry Deployed**:
- ✅ **Schema Registration API**: POST /schemas/subjects/{subject}/versions
- ✅ **Schema Lookup APIs**: 
  - GET /schemas/subjects (list all)
  - GET /schemas/ids/{id} (by global ID)
  - GET /schemas/config (compatibility mode)
- ✅ **Compatibility Checking**: BACKWARD (default), FORWARD, FULL, NONE modes
- ✅ **JSON Schema Validation**: Validates messages against registered schemas

**Infrastructure Updates**:
- **Terraform Module**: Added security variables (security_enabled, security_root_api_key)
- **Helm Templates**: 
  - StatefulSet: HTTPS health probes when TLS enabled
  - ServiceMonitor: HTTPS scheme with insecureSkipVerify for Prometheus
  - TLS secrets auto-generated via Helm genSelfSignedCert
- **Docker Image**: v0.7.0-security-schemas built and pushed to GHCR
...
{"status":"pass","timestamp":"2026-02-05T12:48:41Z","uptime":"1m38.288620015s"}

# Without auth - rejected
curl -sk https://localhost:8443/topics
no API key provided

# With API key - works
curl -sk -H "X-API-Key: 3c78bc92f544273a7772dbcf5bbd8bfc872b103acbf5750ea2b4daf96330a1e2" https://localhost:8443/topics
{"topics":["cluster-test","demo-orders"]}

# Register schema
curl -sk -X POST -H "X-API-Key: ..." -H "Content-Type: application/json" \
  -d '{"schema": "{\"type\":\"object\",\"properties\":{\"order_id\":{\"type\":\"string\"}},\"required\":[\"order_id\"]}"}' \
  https://localhost:8443/schemas/subjects/demo-orders-value/versions
{"id":1,"version":1}

# List schema subjects
curl -sk -H "X-API-Key: ..." https://localhost:8443/schemas/subjects
["demo-orders-value"]
```
...
## Previous Session - EKS Deployment & Cluster Validation

### What Was Accomplished

**Terraform EKS Deployment Issues Fixed**:
- **Problem 1**: "Unauthorized" error when creating Kubernetes resources after EKS cluster creation
- **Root Cause**: EKS 1.28+ uses Access Entry API; cluster creator doesn't auto-get access
- **Fix**: Added `enable_cluster_creator_admin_permissions = true` to EKS module
- **Workaround Applied**: Manually created access entry via AWS CLI for immediate fix

- **Problem 2**: Stale Helm provider plugin
- **Fix**: `rm -rf .terraform/providers && terraform init -upgrade`

- **Problem 3**: Helm repo cache missing
- **Fix**: `helm repo add prometheus-community ... && helm repo update`

**Successful EKS Deployment**:
- EKS cluster: goqueue-dev (K8s 1.31) in ap-south-1
- 3 worker nodes (t3.medium) running
- kube-prometheus-stack deployed with Grafana, Prometheus, Alertmanager
- GoQueue StatefulSet with 3 replicas running

**Cluster Features Validated**:
1. ✅ **Cluster Formation**: All 3 nodes joined cluster successfully
2. ✅ **Controller Election**: goqueue-0 initially elected, then goqueue-1 after failover
3. ✅ **Node Failure Recovery**: Deleted goqueue-0 → rejoined cluster automatically
4. ✅ **Topic Operations**: Created topic with 3 partitions, produced messages
5. ✅ **Message Distribution**: Messages distributed across partitions (round-robin)
6. ✅ **Prometheus Scraping**: All 3 GoQueue pods being scraped (job="goqueue")
7. ✅ **ServiceMonitor + PrometheusRule**: Both CRDs created in goqueue namespace

**M20: Production Metrics Verified**:
- 51 GoQueue-specific metrics exported
- Consumer lag, fetch latency, transaction metrics available
- Cluster metrics: controller elections, node health, partition count
...
{"status":"ok","timestamp":"2026-02-05T11:42:32Z"}

# Create topic
curl -s -X POST http://localhost:8080/topics -H "Content-Type: application/json" \
  -d '{"name":"cluster-test","numPartitions":3}'
{"created":true,"name":"cluster-test","partitions":3}

# Produce messages
curl -s -X POST "http://localhost:8080/topics/cluster-test/messages" \
  -H "Content-Type: application/json" \
  -d '{"messages":[{"value":"msg1"},{"value":"msg2"},{"value":"msg3"}]}'
{"results":[{"partition":0,"offset":0},{"partition":1,"offset":0},{"partition":2,"offset":0}]}

# Check Prometheus targets
curl -s "http://localhost:9090/api/v1/targets" | jq '.data.activeTargets | length'
29

# GoQueue targets healthy
curl -s "http://localhost:9090/api/v1/targets" | jq '.data.activeTargets[] | select(.labels.job=="goqueue")'
# Shows 3 instances, all "up"
```
...
kubectl delete pod goqueue-0 -n goqueue

# Logs show re-election:
# "starting controller election" reason="no controller or controller dead"
# "became controller" epoch=2

# Node rejoins:
# "node recovered, syncing metadata" recovered_node=goqueue-0
# "node joined cluster" node=goqueue-0 cluster_size=2
```

## Previous Session - M21 Security Implementation

### What Was Accomplished

**Terraform Fixes (ROOT CAUSE RESOLUTION)**:
- **Problem**: Previous AI agent disabled Prometheus (`install_prometheus = false`) instead of fixing root cause
- **Root Cause**: ServiceMonitor CRDs don't exist until Prometheus Operator is installed
- **Fix**: Added `helm_release.prometheus_operator` that installs kube-prometheus-stack BEFORE GoQueue
- Added `depends_on` chain: GoQueue depends on Prometheus Operator installation
- Re-enabled `install_prometheus = true` with explanatory comments

**Kubernetes Version Update**:
- Updated EKS cluster from 1.29 to 1.31 (1.29 is extended support = 6x cost)
- Updated in both `deploy/terraform/environments/dev/main.tf` and `deploy/terraform/modules/aws/main.tf`

**M21: Security Implementation (COMPLETE)**:

1. **TLS Layer (`internal/security/tls.go`)**:
   - TLSConfig struct with all standard options
   - DefaultTLSConfig() and DefaultMTLSConfig() functions
   - generateSelfSignedCert() for testing/development
   - NewTLSConfig() builds crypto/tls.Config from our config
   - LoadTLSConfigFromEnv() for Kubernetes environment variables

2. **Authentication Layer (`internal/security/auth.go`)**:
   - APIKey struct with ID, Name, KeyHash, Prefix, Roles, expiry
   - APIKeyManager with in-memory storage (hash-indexed)
   - GenerateKey() creates cryptographically secure keys
   - ValidateKey() checks hash, expiry, revocation
   - AuthMiddleware() extracts and validates API key from header
   - Roles: admin, producer, consumer, readonly
   - Permissions: topic:read, topic:create, message:publish, etc.

3. **ACL Layer (`internal/security/acl.go`)**:
   - ACL struct with Principal, ResourceType, ResourceName, Operation, Allow
   - ACLManager with pattern matching (glob patterns like "orders-*")
   - CheckAccess() evaluates ACLs for a key + resource + operation
   - SecurityManager unifies TLS, Auth, and ACL managers
   - RequirePermission() middleware for route-level authorization
   - RequireTopicAccess() middleware for topic-specific ACLs

4. **Admin API Endpoints**:
   - POST /admin/keys - Create API key
   - GET /admin/keys - List all keys (metadata only)
   - DELETE /admin/keys/{keyID} - Revoke API key
   - POST /admin/acls - Add ACL rule
   - GET /admin/acls - List all ACLs
   - DELETE /admin/acls/{aclID} - Remove ACL rule
   - All protected with RequirePermission("admin:keys") or RequirePermission("admin:acls")

5. **API Server Integration**:
   - SecurityManager field added to Server struct
   - TLS support in Start() method with ListenAndServeTLS
   - Auth middleware applied when security is configured

6. **Helm Chart Updates**:
   - values.yaml: Added security section (tls, clusterTls, auth, rbac)
   - templates/tls-secret.yaml: Self-signed certificate generation
   - templates/api-key-secret.yaml: Root API key storage
   - templates/statefulset.yaml: Security env vars and volume mounts

### Code Changes

**New Files**:
- `internal/security/tls.go` - TLS configuration and cert generation
- `internal/security/auth.go` - API key authentication and RBAC
- `internal/security/acl.go` - Topic-level access control
- `deploy/kubernetes/helm/goqueue/templates/tls-secret.yaml`
- `deploy/kubernetes/helm/goqueue/templates/api-key-secret.yaml`

**Modified Files**:
- `deploy/terraform/environments/dev/main.tf` - K8s version + Prometheus fix
- `deploy/terraform/modules/aws/main.tf` - Added prometheus_operator Helm release
- `deploy/kubernetes/helm/goqueue/values.yaml` - Security configuration
- `deploy/kubernetes/helm/goqueue/templates/statefulset.yaml` - Security env/volumes
- `internal/api/server.go` - Security integration + TLS
- `internal/api/admin_api.go` - API key/ACL management endpoints
...
1. Build and push Docker image with security code
2. Deploy to EKS with terraform
3. Test TLS with curl
4. Test API key authentication
5. Test RBAC permissions

---

## Previous Session - Topic Metadata Sync & Partition API
